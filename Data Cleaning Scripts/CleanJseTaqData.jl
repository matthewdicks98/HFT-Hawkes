...
# classify trades use quote rule and then tick rule
...
function compact(full_df::DataFrame)
    ...
    @showprogress "Compacting..." for i in 1:length(time_stamps)
        ...
        if all_trade_inds != []
            ...
            temp = (current_time_stamp, Date.(current_time_stamp), Time.(current_time_stamp), "TRADE", NaN, NaN, NaN, NaN, vwap_price, cummulative_vol, NaN, NaN, NaN, NaN, NaN, "",1)
            push!(full_compact_df, temp)
...
function compact_3(full_df::DataFrame)
    ...
    @showprogress "Compacting (method 3)..." for i in 1:length(time_stamps)
        ...
        # set storage
        first_trade_ind = -1
        compacted_trades = NaN

        # if trades did occur in the timestamp
        if all_trade_inds != []

            # set the first trade ID to help maintain order later
            first_trade_ind = findfirst(x -> x == "TRADE", time_stamp_events[:,:eventType])
            ...
            # push to the DataFrame (using 1 for id as a place holder)
            compacted_trades = (current_time_stamp, Date.(current_time_stamp), Time.(current_time_stamp), "TRADE", NaN, NaN, NaN, NaN, vwap_price, cummulative_vol, NaN, NaN, NaN, NaN, NaN, "",1)
        ...
        if !isnothing(best_bid_ind) && !isnothing(best_ask_ind)
            ...
            if best_bid_ind < best_ask_ind
                ...
                last_best_bid = time_stamp_events[best_bid_ind,:]
                last_best_ask = time_stamp_events[best_ask_ind,:]

                # find where to put the trade
                if first_trade_ind < best_bid_ind

                    # need to check if there was a first trade (will always go in here if no trade since -1)
                    if first_trade_ind != -1

                        push!(full_compact_df, compacted_trades)

                    end

                    push!(full_compact_df, last_best_bid)
                    push!(full_compact_df, last_best_ask)

                elseif first_trade_ind < best_ask_ind

                    push!(full_compact_df, last_best_bid)
                    push!(full_compact_df, compacted_trades)
                    push!(full_compact_df, last_best_ask)

                else # trade was the last thing to happen
            ...
            elseif best_ask_ind < best_bid_ind
                ...
                # push ask before bid
                last_best_bid = time_stamp_events[best_ask_ind,:]
                last_best_ask = time_stamp_events[best_bid_ind,:]

                # find where to put the trade
                if first_trade_ind < best_ask_ind
                ...
                elseif first_trade_ind < best_bid_ind
...
function makeCleanTaqData(ticker::String)
    ...
    compact_full_df = compact_3(created_full_df)  # need to do compacting here
